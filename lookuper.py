...
lookup.register(Sequence, lambda target, data: (lookup_data(data, target)))
lookup_data = singledispatch(
    lambda data, targets: (() if targets else (data,))
)
lookup_data.register(
    Mapping,
    lambda data, targets: (
        (
            k
...
lookup_data.register(str, lambda data, targets: (() if targets else (data,)))
lookup_index = singledispatch(lambda index, data: ())
lookup_index.register(
    Callable,
    lambda index, data: (
        d for i, v in enumerate(data) for d in index(i, v, data)
    ),
)
lookup_index.register(
    int, lambda index, data: ((data[index],) if index < len(data) else ())
)
lookup_index.register(type(STAR), lambda index, data: (data))
lookup_index.register(
    type(GLOBSTAR),
    lambda index, data: (
        data,
...
lookup_key = singledispatch(
    lambda key, data: ((data[key],) if key in data else ())
)
lookup_key.register(
    Callable,
    lambda key, data: (d for k, v in data.items() for d in key(k, v, data)),
)
lookup_key.register(
    RePattern, lambda key, data: (lookup_key(match_key(key.match), data))
)
lookup_key.register(type(STAR), lambda key, data: (data.values()))
lookup_key.register(
    type(GLOBSTAR),
    lambda key, data: (
        data,
...
lookup_value = singledispatch(
    lambda value, data: ((value,) if value in data else ())
)
lookup_value.register(
    Callable, lambda value, data: (d for v in data for d in value(v, v, data))
)
lookup_value.register(
    RePattern, lambda value, data: (lookup_value(match_key(value.match), data))
)
lookup_value.register(type(STAR), lambda value, data: (data))
lookup_value.register(
    type(GLOBSTAR),
    lambda value, data: (
        data,
...
match_key = singledispatch(lambda key: (match_key(lambda k: k == key)))
match_key.register(
    Callable, lambda key: (lambda k, v, _: (v,) if key(k) else ())
)
match_value = singledispatch(lambda value: (match_value(lambda v: v == value)))
